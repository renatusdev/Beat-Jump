...
public class AddRotationAnim : MonoBehaviour
{
    ...
    [Range(0,1000)] public int magnitude;
    public int duration;
    public bool loop;
    public bool atStart;
    ...
    void Start()
    {
        ...
        if (loop)
            LeanTween.rotate(this.gameObject, CreateDirection() * magnitude, duration).setEase(curve).setLoopPingPong();
        else
            LeanTween.rotate(this.gameObject, CreateDirection() * magnitude, duration).setEase(curve);
    }
